...
def test_upload_with_view(df, formatter, schema):
    ...
    with interface.get_db_conn() as conn:
        cursor = conn.cursor()
        cursor.execute(
            f"create or replace view {interface.schema_name}.{interface.table_name}_view as select * from {interface.schema_name}.{interface.table_name}"
        )
        cursor.execute(
            f"create or replace view {interface.schema_name}.{interface.table_name}_view2 as select * from {interface.schema_name}.{interface.table_name}_view"
        )
        conn.commit()
    interface = upload(
        source=df.copy(),  # needed for the comparison later
    ...
    with interface.get_db_conn() as conn:
        df_out = pandas.read_sql(
            f"select * from {interface.schema_name}.{interface.table_name}_view2 order by a",
            conn,
        )

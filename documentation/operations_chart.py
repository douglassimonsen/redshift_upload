...
dot.node(
    "I",
    "Load data into intermediate data store (S3)",
    style="filled",
    fillcolor="#add8e6",
)
...
dot.node(
    "N", "<Return interface object for reuse<br/>[If <b>close_on_end</b> is False]>"
)
...
dot.node(
    "legend",
    'The flow of the upload process. Nodes in light blue are the core components that run every time.   \nOther nodes can be activated by changing the upload_options dictionary. Options are run left   \nto right (that is, "Recreate views from local data" will run before "Clean up S3 files".   ',
    shape="rectangle",
    lp="0,0!",
)
